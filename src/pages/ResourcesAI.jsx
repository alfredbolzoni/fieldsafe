...
function MessageBubble({ msg }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: isUser ? 'flex-end' : 'flex-start', marginBottom: 16, alignItems: 'flex-start' }}>
      {!isUser && (
        <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg, var(--primary) 0%, #0d5fa3 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginRight: 10, marginTop: 2, boxShadow: '0 2px 8px rgba(26,111,175,0.3)' }}>
      ...
      <div style={{
        maxWidth: '80%',
        background: isUser ? 'linear-gradient(135deg, var(--primary) 0%, #0d5fa3 100%)' : 'var(--surface)',
        ...
        fontSize: 13,
        lineHeight: 1.7,
        boxShadow: isUser ? '0 2px 12px rgba(26,111,175,0.25)' : 'var(--shadow-sm)',
      }}>
        {msg.content.split('\n').map((line, i) => {
          if (line.startsWith('**') && line.endsWith('**')) return <div key={i} style={{ fontWeight: 700, marginTop: i > 0 ? 10 : 0, marginBottom: 4, color: isUser ? 'white' : 'var(--primary)' }}>{line.replace(/\*\*/g, '')}</div>
          if (line.startsWith('- ') || line.startsWith('• ')) return <div key={i} style={{ paddingLeft: 16, position: 'relative', marginBottom: 3 }}><span style={{ position: 'absolute', left: 4, color: isUser ? 'rgba(255,255,255,0.7)' : 'var(--primary)' }}>•</span>{line.replace(/^[-•] /, '')}</div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '20px 16px', minHeight: 0 }}>
              ...
              {loading && (
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 16 }}>
                  <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg, var(--primary) 0%, #0d5fa3 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 2px 8px rgba(26,111,175,0.3)' }}>
                    <svg width="14" height="14" viewBox="0 0 15 15" fill="none"><path d="M7.5 1L14 4.5L7.5 8L1 4.5L7.5 1Z" stroke="white" strokeWidth="1.3" strokeLinejoin="round"/></svg>
                  </div>
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '4px 18px 18px 18px', padding: '12px 16px', display: 'flex', gap: 5, alignItems: 'center' }}>
                    {[0,1,2].map(i => <div key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--primary)', animation: 'pulse 1.4s ease-in-out infinite', animationDelay: `${i * 0.2}s`, opacity: 0.6 }} />)}
                  </div>
                </div>
              )}
              <div ref={bottomRef} />
            ...
            {RESOURCES.map((r, i) => (
              <a key={i} href={r.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', transition: 'all 0.15s' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--primary)'; e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 4px 12px rgba(26,111,175,0.12)' }}
